...
_L6:
		...
		System.out.println((new StringBuilder("÷¥––SQL”Ôæ‰:")).append(tSQL).toString());
		if (tExeSQL.execUpdateSQL(tSQL))
			continue; /* Loop/switch isn't completed */
		CError.buildErr(this, "÷¥––∏¸–¬”Ôæ‰ ß∞‹");
		conn.rollback();
		conn.close();
		return false;
_L7:
...
_L8:
		...
		if (tExeSQL.execUpdateSQL(tSQL))
			continue; /* Loop/switch isn't completed */
		CError.buildErr(this, (new StringBuilder("÷¥––≤Â»Î”Ôæ‰ ß∞‹‘≠“Ú£∫")).append(tExeSQL.mErrors.getError(0).errorMessage).toString());
		conn.rollback();
		conn.close();
		return false;
_L5:
...
_L11:
		...
		CError.buildErr(this, "÷¥––…æ≥˝£¨≤Â»Î”Ôæ‰ ß∞‹");
		try
		{
			conn.rollback();
		}
