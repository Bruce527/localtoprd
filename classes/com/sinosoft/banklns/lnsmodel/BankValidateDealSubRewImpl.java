	...
	implements BankValidateDealIface
{
	...
	private String showFormate1()
	{
		...
		for (int i = 1; errors != null && i <= errors.size(); i++)
			if (i <= 1)
				temp = (new StringBuilder(String.valueOf(temp))).append("æ ¡éªŒæœªé?šè¿‡é¡¹å¦‚ä¸?:\n").append(i).append("ã€?").append((String)errors.get(i)).append("\n").toString();
			else
				temp = (new StringBuilder(String.valueOf(temp))).append(i).append("ã€?").append((String)errors.get(i)).append("\n").toString();
