...
// Source File Name:   LNPCalModeDB.java
...
public class LNPCalModeDB extends LNPCalModeSchema
{
	...
	{
		LNPCalModeSchema tSchema = getSchema();
		...
		} else
		{
			...
			tError.moduleName = "LNPCalModeDB";
	...
	public boolean update()
	{
		...
		try
		{
			...
			if (getCalSQL() == null || getCalSQL().equals("null"))
				pstmt.setNull(4, 12);
			else
				pstmt.setString(4, getCalSQL());
	...
	public boolean getInfo()
	{
		...
		if (!mflag)
			con = DBConnPool.getConnection();
		int i;
		pstmt = con.prepareStatement("SELECT * FROM LNPCalMode WHERE  CalCode = ?", 1003, 1007);
		if (getCalCode() == null || getCalCode().equals("null"))
			pstmt.setNull(1, 12);
		else
			pstmt.setString(1, getCalCode());
...
label0:
	...
	public LNPCalModeSet query()
	{
		Statement stmt = null;
		ResultSet rs = null;
		LNPCalModeSet aLNPCalModeSet = new LNPCalModeSet();
		if (!mflag)
			con = DBConnPool.getConnection();
		try
		{
			stmt = con.createStatement(1003, 1007);
			SQLString sqlObj = new SQLString("LNPCalMode");
			LNPCalModeSchema aSchema = getSchema();
			sqlObj.setSQL(5, aSchema);
			String sql = sqlObj.getSQL();
			rs = stmt.executeQuery(sql);
			int i = 0;
			LNPCalModeSchema s1;
			for (; rs.next(); aLNPCalModeSet.add(s1))
			{
				i++;
				s1 = new LNPCalModeSchema();
				s1.setSchema(rs, i);
			}
	...
	public LNPCalModeSet query(int nStart, int nCount)
	{
		...
		try
		{
			...
			while (rs.next()) 
			{
				...
				if (i >= nStart + nCount)
					break;
				LNPCalModeSchema s1 = new LNPCalModeSchema();
				s1.setSchema(rs, i);
				aLNPCalModeSet.add(s1);
			}
			try
			{
				rs.close();
...
label0:
		{
			...
			try
			{
				...
				String sql = (new StringBuilder("update LNPCalMode ")).append(sqlObj.getUpdPart()).append(" where ").append(strWherePart).toString();
	...
	{
		int tCount = 0;
		LNPCalModeSet tLNPCalModeSet = new LNPCalModeSet();
		...
		{
			tCount = 1;
			tLNPCalModeSchema = new LNPCalModeSchema();
			tLNPCalModeSchema.setSchema(mResultSet, 1);
			tLNPCalModeSet.add(tLNPCalModeSchema);
			while (tCount++ < 5000) 
				if (mResultSet.next())
				{
					tLNPCalModeSchema = new LNPCalModeSchema();
