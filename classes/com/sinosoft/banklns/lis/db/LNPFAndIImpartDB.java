...
// Source File Name:   LNPFAndIImpartDB.java
...
public class LNPFAndIImpartDB extends LNPFAndIImpartSchema
{
	...
	{
		LNPFAndIImpartSchema tSchema = getSchema();
		...
		} else
		{
			...
			tError.moduleName = "LNPFAndIImpartDB";
	...
	public boolean getInfo()
	{
		...
		if (!mflag)
			con = DBConnPool.getConnection();
		int i;
		pstmt = con.prepareStatement("SELECT * FROM LNPFAndIImpart WHERE  ImpartId = ?", 1003, 1007);
		if (getImpartId() == null || getImpartId().equals("null"))
			pstmt.setNull(1, 12);
		else
			pstmt.setString(1, getImpartId());
...
label0:
	...
	public LNPFAndIImpartSet query()
	{
		Statement stmt = null;
		ResultSet rs = null;
		LNPFAndIImpartSet aLNPFAndIImpartSet = new LNPFAndIImpartSet();
		if (!mflag)
			con = DBConnPool.getConnection();
		try
		{
			stmt = con.createStatement(1003, 1007);
			SQLString sqlObj = new SQLString("LNPFAndIImpart");
			LNPFAndIImpartSchema aSchema = getSchema();
			sqlObj.setSQL(5, aSchema);
			String sql = sqlObj.getSQL();
			rs = stmt.executeQuery(sql);
			int i = 0;
			LNPFAndIImpartSchema s1;
			for (; rs.next(); aLNPFAndIImpartSet.add(s1))
			{
				i++;
				s1 = new LNPFAndIImpartSchema();
				s1.setSchema(rs, i);
			}
	...
	public LNPFAndIImpartSet query(int nStart, int nCount)
	{
		...
		try
		{
			...
			while (rs.next()) 
			{
				...
				if (i >= nStart + nCount)
					break;
				LNPFAndIImpartSchema s1 = new LNPFAndIImpartSchema();
				s1.setSchema(rs, i);
				aLNPFAndIImpartSet.add(s1);
			}
			try
			{
				rs.close();
...
label0:
		{
			...
			try
			{
				...
				String sql = (new StringBuilder("update LNPFAndIImpart ")).append(sqlObj.getUpdPart()).append(" where ").append(strWherePart).toString();
	...
	{
		int tCount = 0;
		LNPFAndIImpartSet tLNPFAndIImpartSet = new LNPFAndIImpartSet();
		LNPFAndIImpartSchema tLNPFAndIImpartSchema = null;
		if (mResultSet == null)
		{
			CError tError = new CError();
			tError.moduleName = "LNPFAndIImpartDB";
		...
		{
			tCount = 1;
			tLNPFAndIImpartSchema = new LNPFAndIImpartSchema();
			tLNPFAndIImpartSchema.setSchema(mResultSet, 1);
			tLNPFAndIImpartSet.add(tLNPFAndIImpartSchema);
			while (tCount++ < 5000) 
				if (mResultSet.next())
				{
					tLNPFAndIImpartSchema = new LNPFAndIImpartSchema();
