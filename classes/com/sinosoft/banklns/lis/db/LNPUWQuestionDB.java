...
// Source File Name:   LNPUWQuestionDB.java
...
public class LNPUWQuestionDB extends LNPUWQuestionSchema
{
	...
	{
		LNPUWQuestionSchema tSchema = getSchema();
		...
		} else
		{
			...
			tError.moduleName = "LNPUWQuestionDB";
	...
	public boolean getInfo()
	{
		...
		if (!mflag)
			con = DBConnPool.getConnection();
		int i;
		pstmt = con.prepareStatement("SELECT * FROM LNPUWQuestion WHERE  ContNo = ?", 1003, 1007);
		if (getContNo() == null || getContNo().equals("null"))
			pstmt.setNull(1, 12);
		else
			pstmt.setString(1, getContNo());
...
label0:
	...
	public LNPUWQuestionSet query()
	{
		Statement stmt = null;
		ResultSet rs = null;
		LNPUWQuestionSet aLNPUWQuestionSet = new LNPUWQuestionSet();
		if (!mflag)
			con = DBConnPool.getConnection();
		try
		{
			stmt = con.createStatement(1003, 1007);
			SQLString sqlObj = new SQLString("LNPUWQuestion");
			LNPUWQuestionSchema aSchema = getSchema();
			sqlObj.setSQL(5, aSchema);
			String sql = sqlObj.getSQL();
			rs = stmt.executeQuery(sql);
			int i = 0;
			LNPUWQuestionSchema s1;
			for (; rs.next(); aLNPUWQuestionSet.add(s1))
			{
				i++;
				s1 = new LNPUWQuestionSchema();
				s1.setSchema(rs, i);
			}
	...
	public LNPUWQuestionSet query(int nStart, int nCount)
	{
		...
		try
		{
			...
			while (rs.next()) 
			{
				...
				if (i >= nStart + nCount)
					break;
				LNPUWQuestionSchema s1 = new LNPUWQuestionSchema();
				s1.setSchema(rs, i);
				aLNPUWQuestionSet.add(s1);
			}
			try
			{
				rs.close();
...
label0:
		{
			...
			try
			{
				...
				String sql = (new StringBuilder("update LNPUWQuestion ")).append(sqlObj.getUpdPart()).append(" where ").append(strWherePart).toString();
	...
	{
		int tCount = 0;
		LNPUWQuestionSet tLNPUWQuestionSet = new LNPUWQuestionSet();
		LNPUWQuestionSchema tLNPUWQuestionSchema = null;
		if (mResultSet == null)
		{
			CError tError = new CError();
			tError.moduleName = "LNPUWQuestionDB";
		...
		{
			tCount = 1;
			tLNPUWQuestionSchema = new LNPUWQuestionSchema();
			tLNPUWQuestionSchema.setSchema(mResultSet, 1);
			tLNPUWQuestionSet.add(tLNPUWQuestionSchema);
			while (tCount++ < 5000) 
				if (mResultSet.next())
				{
					tLNPUWQuestionSchema = new LNPUWQuestionSchema();
