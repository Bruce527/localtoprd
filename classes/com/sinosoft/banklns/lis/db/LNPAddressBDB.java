...
// Source File Name:   LNPAddressBDB.java
...
public class LNPAddressBDB extends LNPAddressBSchema
{
	...
	public LNPAddressBDB()
	{
		...
		mErrors = new CErrors();
		con = null;
		db = new DBOper("LNPAddressB");
		mflag = false;
	}

	public boolean insert()
	{
		LNPAddressBSchema tSchema = getSchema();
	...
	public boolean getInfo()
	{
		...
		SQLString sqlObj = new SQLString("LNPAddressB");
		LNPAddressBSchema aSchema = getSchema();
...
label0:
	...
	public LNPAddressBSet query()
	{
		Statement stmt = null;
		ResultSet rs = null;
		LNPAddressBSet aLNPAddressBSet = new LNPAddressBSet();
		if (!mflag)
			con = DBConnPool.getConnection();
		try
		{
			...
			rs = stmt.executeQuery(sql);
			int i = 0;
			LNPAddressBSchema s1;
			for (; rs.next(); aLNPAddressBSet.add(s1))
			{
				i++;
				s1 = new LNPAddressBSchema();
				s1.setSchema(rs, i);
			}
	...
	public LNPAddressBSet query(int nStart, int nCount)
	{
		...
		try
		{
			...
			while (rs.next()) 
			{
				...
				if (i >= nStart + nCount)
					break;
				LNPAddressBSchema s1 = new LNPAddressBSchema();
				s1.setSchema(rs, i);
...
label0:
		{
			...
			try
			{
				...
				String sql = (new StringBuilder("update LNPAddressB ")).append(sqlObj.getUpdPart()).append(" where ").append(strWherePart).toString();
