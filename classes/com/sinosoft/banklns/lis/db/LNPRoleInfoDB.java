...
// Source File Name:   LNPRoleInfoDB.java
...
public class LNPRoleInfoDB extends LNPRoleInfoSchema
{
	...
	{
		LNPRoleInfoSchema tSchema = getSchema();
		...
		} else
		{
			...
			tError.moduleName = "LNPRoleInfoDB";
	...
	public boolean update()
	{
		...
		try
		{
			...
			if (getVarc3() == null || getVarc3().equals("null"))
				...
				pstmt.setString(14, getVarc3());
			if (getID() == null || getID().equals("null"))
				pstmt.setNull(15, 12);
			else
				pstmt.setString(15, getID());
			pstmt.executeUpdate();
			pstmt.close();
		}
		catch (Exception ex)
	...
	public boolean getInfo()
	{
		...
		if (!mflag)
			con = DBConnPool.getConnection();
		int i;
		pstmt = con.prepareStatement("SELECT * FROM LNPRoleInfo WHERE  ID = ?", 1003, 1007);
		if (getID() == null || getID().equals("null"))
			pstmt.setNull(1, 12);
		else
			pstmt.setString(1, getID());
...
label0:
	...
	public LNPRoleInfoSet query()
	{
		Statement stmt = null;
		ResultSet rs = null;
		LNPRoleInfoSet aLNPRoleInfoSet = new LNPRoleInfoSet();
		if (!mflag)
			con = DBConnPool.getConnection();
		try
		{
			stmt = con.createStatement(1003, 1007);
			SQLString sqlObj = new SQLString("LNPRoleInfo");
			LNPRoleInfoSchema aSchema = getSchema();
			sqlObj.setSQL(5, aSchema);
			String sql = sqlObj.getSQL();
			rs = stmt.executeQuery(sql);
			int i = 0;
			LNPRoleInfoSchema s1;
			for (; rs.next(); aLNPRoleInfoSet.add(s1))
			{
				i++;
				s1 = new LNPRoleInfoSchema();
				s1.setSchema(rs, i);
			}
	...
	public LNPRoleInfoSet query(int nStart, int nCount)
	{
		...
		try
		{
			...
			while (rs.next()) 
			{
				...
				if (i >= nStart + nCount)
					break;
				LNPRoleInfoSchema s1 = new LNPRoleInfoSchema();
				s1.setSchema(rs, i);
				aLNPRoleInfoSet.add(s1);
			}
			try
			{
				rs.close();
...
label0:
		{
			...
			try
			{
				...
				String sql = (new StringBuilder("update LNPRoleInfo ")).append(sqlObj.getUpdPart()).append(" where ").append(strWherePart).toString();
	...
	{
		int tCount = 0;
		LNPRoleInfoSet tLNPRoleInfoSet = new LNPRoleInfoSet();
		LNPRoleInfoSchema tLNPRoleInfoSchema = null;
		if (mResultSet == null)
		{
			CError tError = new CError();
			tError.moduleName = "LNPRoleInfoDB";
		...
		{
			tCount = 1;
			tLNPRoleInfoSchema = new LNPRoleInfoSchema();
			tLNPRoleInfoSchema.setSchema(mResultSet, 1);
			tLNPRoleInfoSet.add(tLNPRoleInfoSchema);
			while (tCount++ < 5000) 
				if (mResultSet.next())
				{
					tLNPRoleInfoSchema = new LNPRoleInfoSchema();
