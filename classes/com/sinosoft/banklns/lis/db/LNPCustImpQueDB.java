...
// Source File Name:   LNPCustImpQueDB.java
...
public class LNPCustImpQueDB extends LNPCustImpQueSchema
{
	...
	{
		LNPCustImpQueSchema tSchema = getSchema();
	...
	public boolean getInfo()
	{
		...
		if (!mflag)
			con = DBConnPool.getConnection();
		int i;
		pstmt = con.prepareStatement("SELECT * FROM LNPCustImpQue WHERE  Questionid = ? AND InnerOrder = ?", 1003, 1007);
		if (getQuestionid() == null || getQuestionid().equals("null"))
			pstmt.setNull(1, 12);
		else
			pstmt.setString(1, getQuestionid());
...
label0:
	...
	public LNPCustImpQueSet query()
	{
		Statement stmt = null;
		ResultSet rs = null;
		LNPCustImpQueSet aLNPCustImpQueSet = new LNPCustImpQueSet();
		if (!mflag)
			con = DBConnPool.getConnection();
		try
		{
			stmt = con.createStatement(1003, 1007);
			SQLString sqlObj = new SQLString("LNPCustImpQue");
			LNPCustImpQueSchema aSchema = getSchema();
			sqlObj.setSQL(5, aSchema);
			String sql = sqlObj.getSQL();
			rs = stmt.executeQuery(sql);
			int i = 0;
			LNPCustImpQueSchema s1;
			for (; rs.next(); aLNPCustImpQueSet.add(s1))
			{
				i++;
				s1 = new LNPCustImpQueSchema();
				s1.setSchema(rs, i);
			}
	...
	public LNPCustImpQueSet query(int nStart, int nCount)
	{
		...
		try
		{
			...
			while (rs.next()) 
			{
				...
				if (i >= nStart + nCount)
					break;
				LNPCustImpQueSchema s1 = new LNPCustImpQueSchema();
				s1.setSchema(rs, i);
...
label0:
		{
			...
			try
			{
				...
				String sql = (new StringBuilder("update LNPCustImpQue ")).append(sqlObj.getUpdPart()).append(" where ").append(strWherePart).toString();
	...
	{
		int tCount = 0;
		LNPCustImpQueSet tLNPCustImpQueSet = new LNPCustImpQueSet();
		LNPCustImpQueSchema tLNPCustImpQueSchema = null;
		if (mResultSet == null)
		{
			CError tError = new CError();
			tError.moduleName = "LNPCustImpQueDB";
		...
		{
			tCount = 1;
			tLNPCustImpQueSchema = new LNPCustImpQueSchema();
			tLNPCustImpQueSchema.setSchema(mResultSet, 1);
			tLNPCustImpQueSet.add(tLNPCustImpQueSchema);
			while (tCount++ < 5000) 
				if (mResultSet.next())
				{
					tLNPCustImpQueSchema = new LNPCustImpQueSchema();
