...
// Source File Name:   LNPCustImpResultBDB.java
...
public class LNPCustImpResultBDB extends LNPCustImpResultBSchema
{
	...
	{
		LNPCustImpResultBSchema tSchema = getSchema();
		if (!db.insert(tSchema))
		{
			mErrors.copyAllErrors(db.mErrors);
			CError tError = new CError();
			tError.moduleName = "LNPCustImpResultBDB";
	...
	public boolean getInfo()
	{
		...
		SQLString sqlObj = new SQLString("LNPCustImpResultB");
		LNPCustImpResultBSchema aSchema = getSchema();
...
label0:
	...
	public LNPCustImpResultBSet query()
	{
		Statement stmt = null;
		ResultSet rs = null;
		LNPCustImpResultBSet aLNPCustImpResultBSet = new LNPCustImpResultBSet();
		if (!mflag)
			con = DBConnPool.getConnection();
		try
		{
			...
			rs = stmt.executeQuery(sql);
			int i = 0;
			LNPCustImpResultBSchema s1;
			for (; rs.next(); aLNPCustImpResultBSet.add(s1))
			{
				i++;
				s1 = new LNPCustImpResultBSchema();
				s1.setSchema(rs, i);
			}
	...
	public LNPCustImpResultBSet query(int nStart, int nCount)
	{
		...
		try
		{
			...
			while (rs.next()) 
			{
				...
				if (i >= nStart + nCount)
					break;
				LNPCustImpResultBSchema s1 = new LNPCustImpResultBSchema();
				s1.setSchema(rs, i);
				aLNPCustImpResultBSet.add(s1);
			}
			try
			{
				rs.close();
...
label0:
		{
			...
			try
			{
				...
				String sql = (new StringBuilder("update LNPCustImpResultB ")).append(sqlObj.getUpdPart()).append(" where ").append(strWherePart).toString();
