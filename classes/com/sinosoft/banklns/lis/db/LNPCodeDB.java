...
// Source File Name:   LNPCodeDB.java
...
public class LNPCodeDB extends LNPCodeSchema
{
	...
	{
		LNPCodeSchema tSchema = getSchema();
	...
	public boolean getInfo()
	{
		...
		if (!mflag)
			con = DBConnPool.getConnection();
		int i;
		pstmt = con.prepareStatement("SELECT * FROM LNPCode WHERE  CodeType = ? AND Code = ?", 1003, 1007);
		if (getCodeType() == null || getCodeType().equals("null"))
			pstmt.setNull(1, 12);
		else
			pstmt.setString(1, getCodeType());
...
label0:
	...
	public LNPCodeSet query()
	{
		Statement stmt = null;
		ResultSet rs = null;
		LNPCodeSet aLNPCodeSet = new LNPCodeSet();
		if (!mflag)
			con = DBConnPool.getConnection();
		try
		{
			stmt = con.createStatement(1003, 1007);
			SQLString sqlObj = new SQLString("LNPCode");
			LNPCodeSchema aSchema = getSchema();
			sqlObj.setSQL(5, aSchema);
			String sql = sqlObj.getSQL();
			rs = stmt.executeQuery(sql);
			int i = 0;
			LNPCodeSchema s1;
			for (; rs.next(); aLNPCodeSet.add(s1))
			{
				i++;
				s1 = new LNPCodeSchema();
				s1.setSchema(rs, i);
			}
	...
	public LNPCodeSet query(int nStart, int nCount)
	{
		...
		try
		{
			...
			while (rs.next()) 
			{
				...
				if (i >= nStart + nCount)
					break;
				LNPCodeSchema s1 = new LNPCodeSchema();
				s1.setSchema(rs, i);
				aLNPCodeSet.add(s1);
			}
			try
...
label0:
		{
			...
			try
			{
				...
				String sql = (new StringBuilder("update LNPCode ")).append(sqlObj.getUpdPart()).append(" where ").append(strWherePart).toString();
	...
	public LNPCodeSet getData()
	{
		...
		LNPCodeSchema tLNPCodeSchema = null;
		if (mResultSet == null)
		{
			CError tError = new CError();
			tError.moduleName = "LNPCodeDB";
		...
		{
			tCount = 1;
			tLNPCodeSchema = new LNPCodeSchema();
			tLNPCodeSchema.setSchema(mResultSet, 1);
			tLNPCodeSet.add(tLNPCodeSchema);
			while (tCount++ < 5000) 
				if (mResultSet.next())
				{
					tLNPCodeSchema = new LNPCodeSchema();
