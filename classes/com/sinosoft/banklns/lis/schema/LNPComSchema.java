...
// Source File Name:   LNPComSchema.java
	...
	implements Schema, Cloneable
{
	...
	{
		LNPComSchema cloned = (LNPComSchema)super.clone();
	...
	{
		comCode = aLNPComSchema.getComCode();
		outComCode = aLNPComSchema.getOutComCode();
		name = aLNPComSchema.getName();
		shortName = aLNPComSchema.getShortName();
		address = aLNPComSchema.getAddress();
		zipCode = aLNPComSchema.getZipCode();
		phone = aLNPComSchema.getPhone();
		fax = aLNPComSchema.getFax();
		eMail = aLNPComSchema.getEMail();
		webAddress = aLNPComSchema.getWebAddress();
		satrapName = aLNPComSchema.getSatrapName();
		insuMonitorCode = aLNPComSchema.getInsuMonitorCode();
		insureID = aLNPComSchema.getInsureID();
		signID = aLNPComSchema.getSignID();
		country = aLNPComSchema.getCountry();
		province = aLNPComSchema.getProvince();
		city = aLNPComSchema.getCity();
		comNature = aLNPComSchema.getComNature();
		validCode = aLNPComSchema.getValidCode();
		sign = aLNPComSchema.getSign();
	}
	...
	public boolean setSchema(ResultSet rs, int i)
	{
		...
		{
			System.out.println("数据库中的LNPCom表字段个数和Schema中的字段个数不一致，或者执行db.executeQuery查询时没有使用select * from tables");
			CError tError = new CError();
			tError.moduleName = "LNPComSchema";
	...
	{
		LNPComSchema aLNPComSchema = new LNPComSchema();
		aLNPComSchema.setSchema(this);
		return aLNPComSchema;
	}

	public LNPComDB getDB()
	{
		LNPComDB aDBOper = new LNPComDB();
	...
	public boolean equals(Object otherObject)
	{
		...
		if (getClass() != otherObject.getClass())
			return false;
		LNPComSchema other = (LNPComSchema)otherObject;
