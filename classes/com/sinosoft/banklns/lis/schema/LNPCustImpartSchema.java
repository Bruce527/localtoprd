...
// Source File Name:   LNPCustImpartSchema.java
	...
	implements Schema, Cloneable
{
	...
	{
		LNPCustImpartSchema cloned = (LNPCustImpartSchema)super.clone();
	...
	{
		impartId = aLNPCustImpartSchema.getImpartId();
		proposalType = aLNPCustImpartSchema.getProposalType();
		impartName = aLNPCustImpartSchema.getImpartName();
		impartVer = aLNPCustImpartSchema.getImpartVer();
		impartContent = aLNPCustImpartSchema.getImpartContent();
		questionSum = aLNPCustImpartSchema.getQuestionSum();
		operator = aLNPCustImpartSchema.getOperator();
		manageCom = aLNPCustImpartSchema.getManageCom();
		makeTime = aLNPCustImpartSchema.getMakeTime();
		makeDate = fDate.getDate(aLNPCustImpartSchema.getMakeDate());
		modifyDate = fDate.getDate(aLNPCustImpartSchema.getModifyDate());
		modifyTime = aLNPCustImpartSchema.getModifyTime();
		p1 = aLNPCustImpartSchema.getP1();
		p2 = aLNPCustImpartSchema.getP2();
		p3 = aLNPCustImpartSchema.getP3();
		p4 = aLNPCustImpartSchema.getP4();
	}
	...
	public boolean setSchema(ResultSet rs, int i)
	{
		...
		{
			System.out.println("数据库中的LNPCustImpart表字段个数和Schema中的字段个数不一致，或�?�执行db.executeQuery查询时没有使用select * from tables");
	...
	{
		LNPCustImpartSchema aLNPCustImpartSchema = new LNPCustImpartSchema();
		aLNPCustImpartSchema.setSchema(this);
		return aLNPCustImpartSchema;
	}

	public LNPCustImpartDB getDB()
	{
		LNPCustImpartDB aDBOper = new LNPCustImpartDB();
	...
	public boolean decode(String strMessage)
	{
		try
		{
			...
			proposalType = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 2, "|");
			impartName = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 3, "|");
			impartVer = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 4, "|");
			impartContent = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 5, "|");
			questionSum = (new Integer(ChgData.chgNumericStr(StrTool.getStr(strMessage, 6, "|")))).intValue();
			...
			p1 = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 13, "|");
			p2 = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 14, "|");
			p3 = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 15, "|");
	...
	public boolean equals(Object otherObject)
	{
		...
		if (getClass() != otherObject.getClass())
			return false;
		LNPCustImpartSchema other = (LNPCustImpartSchema)otherObject;
	...
	public int getFieldIndex(String strFieldName)
	{
		...
		if (strFieldName.equals("p3"))
			return 14;
		return !strFieldName.equals("p4") ? -1 : 15;
	}
