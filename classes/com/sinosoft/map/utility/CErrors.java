	...
	implements Cloneable
{
	...
	{
		content = (new StringBuilder("（一共发生 ")).append(vErrors.size()).append(" 个错误）\n").toString();
		...
		{
			CError tError = (CError)vErrors.get(i);
			System.out.println((new StringBuilder("***********")).append(tError.errorMessage).append("----------").append(i).toString());
			if (tError.errorType.equals("1"))
			{
				System.out.println((new StringBuilder("*****666****")).append(tError.errorType).toString());
				forbitContent = (new StringBuilder(String.valueOf(forbitContent))).append("  ").append("（错误编码：").append(tError.errorNo).append("）").append(tError.errorMessage).append("\n").toString();
			} else
			if (tError.errorType.equals("3"))
				needSelectContent = (new StringBuilder(String.valueOf(needSelectContent))).append("  ").append("（错误编码：").append(tError.errorNo).append("）").append(tError.errorMessage).append("\n").toString();
			else
			if (tError.errorType.equals("2"))
				allowContent = (new StringBuilder(String.valueOf(allowContent))).append("  ").append("（错误编码：").append(tError.errorNo).append("）").append(tError.errorMessage).append("\n").toString();
			else
				unknowContent = (new StringBuilder(String.valueOf(unknowContent))).append("  ").append("（错误编码：意外错误）").append(tError.errorMessage).append("\n").toString();
		}

		if (!forbitContent.equals(""))
			content = (new StringBuilder(String.valueOf(content))).append("严重错误如下:\n").append(forbitContent).toString();
		if (!needSelectContent.equals(""))
			content = (new StringBuilder(String.valueOf(content))).append("需要选择的错误如下:\n").append(needSelectContent).toString();
		if (!allowContent.equals(""))
			content = (new StringBuilder(String.valueOf(content))).append("允许出现的错误如下:\n").append(allowContent).toString();
		if (!unknowContent.equals(""))
			content = (new StringBuilder(String.valueOf(content))).append("意外错误如下:\n").append(unknowContent).toString();
		return content;
	}
	...
	public void checkErrors(CErrors cerrors)
	{
		...
		} else
		{
			...
			{
				content = (new StringBuilder("保存成功，但是：")).append(PubFun.changForHTML(ErrorContent)).toString();
				flag = "Success";
			} else
			{
				content = (new StringBuilder("保存失败，原因是：")).append(PubFun.changForHTML(ErrorContent)).toString();
				flag = "Fail";
			}
		}
	}
	...
	public void checkWSErrors(VData v)
	{
		...
		if (type.equals("2"))
		{
			content = (new StringBuilder("保存成功，但是：")).append(PubFun.changForHTML((String)v.get(1))).toString();
			flag = "Success";
		} else
		{
			content = (new StringBuilder("保存失败，原因是：")).append(PubFun.changForHTML((String)v.get(1))).toString();
			flag = "Fail";
		}
	}
