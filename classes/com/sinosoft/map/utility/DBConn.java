	...
	public boolean createConnection()
	{
		...
		if (JUrl.getDBType().equalsIgnoreCase("JBOSS"))
		{
			...
			if (getJBOSSPoolConnection())
				return true;
			System.out.println("/map--------------------jboos conPool is failed��con get from jdbcurlbak-------------------------------");
			JdbcUrlBackUp tJdbcUrlBackUp = new JdbcUrlBackUp();
			JUrl.setDBName(tJdbcUrlBackUp.getDBName());
			JUrl.setDBType(tJdbcUrlBackUp.getDBType());
			JUrl.setIP(tJdbcUrlBackUp.getIP());
...
_L1:
	...
	{
		System.out.println((new StringBuilder("jndi name:jdbc/")).append(JUrl.getDBName()).toString());
		Context tContext = new InitialContext();
		DataSource tDataSource = (DataSource)tContext.lookup((new StringBuilder("java:comp/env/jdbc/")).append(JUrl.getDBName()).toString());
		if (tDataSource == null)
			break MISSING_BLOCK_LABEL_113;
		con = tDataSource.getConnection();
		if (con == null)
			break MISSING_BLOCK_LABEL_102;
		System.out.println("/map--- new Connection from JBOSS Pool ...");
		return true;
		try
		{
