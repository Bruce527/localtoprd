...
// Source File Name:   BirthdayQuery.java
...
public class BirthdayQuery
{
	...
	public boolean doBatch()
	{
		...
		try
		{
			...
			sql = (new StringBuilder(String.valueOf(sql))).append(" where tb_g.dt_birth is not null ").toString();
			...
			if (strmonth.compareTo("12") == 0 && strday.compareTo("03") >= 0)
				sql = (new StringBuilder(String.valueOf(sql))).append(" and (( substring(CONVERT(varchar(24), tb_g.dt_birth, 23),6,10) >= substring(CONVERT(varchar(24),getdate(),23),6,10) and substring(CONVERT(varchar(24), tb_g.dt_birth, 23),6,10) <='12-31') or ( substring(CONVERT(varchar(24), tb_g.dt_birth, 23),6,10) <= substring(CONVERT(varchar(24),getdate()+30,23),6,10) and substring(CONVERT(varchar(24), tb_g.dt_birth, 23),6,10) >='01-01') )").toString();
			else
				sql = (new StringBuilder(String.valueOf(sql))).append(" and substring(CONVERT(varchar(24), tb_g.dt_birth, 23),6,10) >= substring(CONVERT(varchar(24),getdate(),23),6,10) and substring(CONVERT(varchar(24), tb_g.dt_birth, 23),6,10) <= substring(CONVERT(varchar(24),getdate()+30,23),6,10) ").toString();
			...
			sql = (new StringBuilder(String.valueOf(sql))).append(" select  la.agentcode as agentcode ").toString();
			sql = (new StringBuilder(String.valueOf(sql))).append(" from tb_application tb_app ").toString();
			sql = (new StringBuilder(String.valueOf(sql))).append(" INNER JOIN TB_PARTICIPANT_Encrypt tb_p ON  tb_app.cd_administrator = tb_p.cd_administrator AND tb_app.cd_branch = tb_p.cd_branch AND tb_app.cd_participant = tb_p.cd_participant and tb_app.src_cd = tb_p.src_cd ").toString();
			sql = (new StringBuilder(String.valueOf(sql))).append(" inner join   tb_application_broker tb_a_b  on tb_app.cd_administrator=tb_a_b.cd_administrator and tb_app.cd_branch=tb_a_b.cd_branch and tb_app.cd_product=tb_a_b.cd_product  and tb_app.cd_application=tb_a_b.cd_application ").toString();
			sql = (new StringBuilder(String.valueOf(sql))).append(" inner join laagent la  on  '86' + right('0'+ltrim(str(tb_a_b.cd_branch)),2) + right('00000' + ltrim(str(tb_a_b.cd_broker)),6)=la.agentcode ").toString();
			sql = (new StringBuilder(String.valueOf(sql))).append(" where tb_p.dt_birth is not null ").toString();
			sql = (new StringBuilder(String.valueOf(sql))).append(" and la.branchtype='1' ").toString();
			if (strmonth.compareTo("12") == 0 && strday.compareTo("03") >= 0)
				sql = (new StringBuilder(String.valueOf(sql))).append(" and (( substring(CONVERT(varchar(24), tb_p.dt_birth, 23),6,10) >= substring(CONVERT(varchar(24),getdate(),23),6,10) and substring(CONVERT(varchar(24), tb_p.dt_birth, 23),6,10) <='12-31') or ( substring(CONVERT(varchar(24), tb_p.dt_birth, 23),6,10) <= substring(CONVERT(varchar(24),getdate()+30,23),6,10) and substring(CONVERT(varchar(24), tb_p.dt_birth, 23),6,10) >='01-01') )").toString();
			else
				sql = (new StringBuilder(String.valueOf(sql))).append(" and substring(CONVERT(varchar(24), tb_p.dt_birth, 23),6,10) >= substring(CONVERT(varchar(24),getdate(),23),6,10) and substring(CONVERT(varchar(24), tb_p.dt_birth, 23),6,10) <= substring(CONVERT(varchar(24),getdate()+30,23),6,10) ").toString();
			sql = (new StringBuilder(String.valueOf(sql))).append(" union all ").toString();
			sql = (new StringBuilder(String.valueOf(sql))).append(" select la.agentcode as agentcode ").toString();
			sql = (new StringBuilder(String.valueOf(sql))).append(" from lslatncycustm c ").toString();
			sql = (new StringBuilder(String.valueOf(sql))).append(" inner join laagent la on c.operator=la.agentcode ").toString();
			sql = (new StringBuilder(String.valueOf(sql))).append(" where 1=1 and c.Birthday is not null ").toString();
			sql = (new StringBuilder(String.valueOf(sql))).append(" and not exists ( select 1 from  mclientrelation where c.CustomerNo=prospectsno ) ").toString();
			if (strmonth.compareTo("12") == 0 && strday.compareTo("03") >= 0)
				sql = (new StringBuilder(String.valueOf(sql))).append(" and (( substring(CONVERT(varchar(24), c.Birthday, 23),6,10) >= substring(CONVERT(varchar(24),getdate(),23),6,10) and substring(CONVERT(varchar(24), c.Birthday, 23),6,10) <='12-31') or ( substring(CONVERT(varchar(24), c.Birthday, 23),6,10) <= substring(CONVERT(varchar(24),getdate()+30,23),6,10) and substring(CONVERT(varchar(24), c.Birthday, 23),6,10) >='01-01') )").toString();
			else
				sql = (new StringBuilder(String.valueOf(sql))).append(" and substring(CONVERT(varchar(24), c.Birthday, 23),6,10) >= substring(CONVERT(varchar(24),getdate(),23),6,10) and substring(CONVERT(varchar(24), c.Birthday, 23),6,10) <= substring(CONVERT(varchar(24),getdate()+30,23),6,10) ").toString();
			sql = (new StringBuilder(String.valueOf(sql))).append(" and la.branchtype='1' ").toString();
			...
			if (ssrs1 != null && ssrs1.MaxRow > 0)
			{
				for (int i = 1; i <= ssrs1.MaxRow; i++)
				{
					...
					tMPrivateMessageSchema.setMessageType("06");
			...
			String querysql = (new StringBuilder(" select id from MPrivateMessage where branchtype ='1' and messagetype='06' and operator <> '")).append(tDelID).append("'  ").toString();
			SSRS ssrs2 = new SSRS();
