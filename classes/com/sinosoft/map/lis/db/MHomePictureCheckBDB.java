...
// Source File Name:   MHomePictureCheckBDB.java
...
public class MHomePictureCheckBDB extends MHomePictureCheckBSchema
{
	...
	{
		MHomePictureCheckBSchema tSchema = getSchema();
		...
		} else
		{
			...
			tError.moduleName = "MHomePictureCheckBDB";
	...
	public boolean update()
	{
		...
		try
		{
			...
			if (getMakeDate2() == null || getMakeDate2().equals("null"))
				...
				pstmt.setDate(28, Date.valueOf(getMakeDate2()));
	...
	public boolean getInfo()
	{
		...
		if (!mflag)
			con = DBConnPool.getConnection();
		int i;
		pstmt = con.prepareStatement("SELECT * FROM MHomePictureCheckB WHERE  EdorNo = ?", 1003, 1007);
		if (getEdorNo() == null || getEdorNo().equals("null"))
			pstmt.setNull(1, 12);
		else
			pstmt.setString(1, getEdorNo());
...
label0:
	...
	public MHomePictureCheckBSet query()
	{
		Statement stmt = null;
		ResultSet rs = null;
		MHomePictureCheckBSet aMHomePictureCheckBSet = new MHomePictureCheckBSet();
		if (!mflag)
			con = DBConnPool.getConnection();
		try
		{
			stmt = con.createStatement(1003, 1007);
			SQLString sqlObj = new SQLString("MHomePictureCheckB");
			MHomePictureCheckBSchema aSchema = getSchema();
			sqlObj.setSQL(5, aSchema);
			String sql = sqlObj.getSQL();
			rs = stmt.executeQuery(sql);
			int i = 0;
			MHomePictureCheckBSchema s1;
			for (; rs.next(); aMHomePictureCheckBSet.add(s1))
			{
				i++;
				s1 = new MHomePictureCheckBSchema();
				s1.setSchema(rs, i);
			}
	...
	public MHomePictureCheckBSet query(int nStart, int nCount)
	{
		...
		try
		{
			...
			while (rs.next()) 
			{
				...
				if (i >= nStart + nCount)
					break;
				MHomePictureCheckBSchema s1 = new MHomePictureCheckBSchema();
				s1.setSchema(rs, i);
				aMHomePictureCheckBSet.add(s1);
			}
			try
			{
				rs.close();
...
label0:
		{
			...
			try
			{
				...
				String sql = (new StringBuilder("update MHomePictureCheckB ")).append(sqlObj.getUpdPart()).append(" where ").append(strWherePart).toString();
	...
	{
		int tCount = 0;
		MHomePictureCheckBSet tMHomePictureCheckBSet = new MHomePictureCheckBSet();
		MHomePictureCheckBSchema tMHomePictureCheckBSchema = null;
		if (mResultSet == null)
		{
			CError tError = new CError();
			tError.moduleName = "MHomePictureCheckBDB";
		...
		{
			tCount = 1;
			tMHomePictureCheckBSchema = new MHomePictureCheckBSchema();
			tMHomePictureCheckBSchema.setSchema(mResultSet, 1);
			tMHomePictureCheckBSet.add(tMHomePictureCheckBSchema);
			while (tCount++ < 5000) 
				if (mResultSet.next())
				{
					tMHomePictureCheckBSchema = new MHomePictureCheckBSchema();
