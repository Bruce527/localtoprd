...
public class MMenuDB extends MMenuSchema
{
	...
	public boolean update()
	{
		...
		try
		{
			...
			if (getNodeName() == null || getNodeName().equals("null"))
				...
				pstmt.setString(2, getNodeName());
			if (getParentNodeCode() == null || getParentNodeCode().equals("null"))
				pstmt.setNull(3, 12);
			else
				pstmt.setString(3, getParentNodeCode());
			if (getNodeLevel() == null || getNodeLevel().equals("null"))
				pstmt.setNull(4, 12);
			else
				pstmt.setString(4, getNodeLevel());
			...
			if (getNodeCode() == null || getNodeCode().equals("null"))
				...
				pstmt.setString(20, getNodeCode());
			pstmt.executeUpdate();
			pstmt.close();
		}
		catch (Exception ex)
	...
	public boolean getInfo()
	{
		...
		if (!mflag)
			con = DBConnPool.getConnection();
		int i;
		pstmt = con.prepareStatement("SELECT * FROM MMenu WHERE  NodeCode = ?", 1003, 1007);
		if (getNodeCode() == null || getNodeCode().equals("null"))
			pstmt.setNull(1, 12);
		else
			pstmt.setString(1, getNodeCode());
...
label0:
	...
	public MMenuSet query()
	{
		Statement stmt = null;
		ResultSet rs = null;
		MMenuSet aMMenuSet = new MMenuSet();
		if (!mflag)
			con = DBConnPool.getConnection();
		try
		{
			stmt = con.createStatement(1003, 1007);
			SQLString sqlObj = new SQLString("MMenu");
			MMenuSchema aSchema = getSchema();
			sqlObj.setSQL(5, aSchema);
			String sql = sqlObj.getSQL();
			rs = stmt.executeQuery(sql);
			int i = 0;
			MMenuSchema s1;
			for (; rs.next(); aMMenuSet.add(s1))
			{
				i++;
				s1 = new MMenuSchema();
				s1.setSchema(rs, i);
	...
	public MMenuSet query(int nStart, int nCount)
	{
		...
		try
		{
			...
			while (rs.next()) 
			{
				...
				if (i >= nStart + nCount)
					break;
				MMenuSchema s1 = new MMenuSchema();
				s1.setSchema(rs, i);
				aMMenuSet.add(s1);
			}
			try
...
label0:
		{
			...
			try
			{
				...
				String sql = (new StringBuilder("update MMenu ")).append(sqlObj.getUpdPart()).append(" where ").append(strWherePart).toString();
	...
	{
		int tCount = 0;
		MMenuSet tMMenuSet = new MMenuSet();
		MMenuSchema tMMenuSchema = null;
		if (mResultSet == null)
		{
			CError tError = new CError();
			tError.moduleName = "MMenuDB";
		...
		{
			tCount = 1;
			tMMenuSchema = new MMenuSchema();
			tMMenuSchema.setSchema(mResultSet, 1);
			tMMenuSet.add(tMMenuSchema);
			while (tCount++ < 5000) 
				if (mResultSet.next())
				{
					tMMenuSchema = new MMenuSchema();
