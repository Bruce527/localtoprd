...
// Source File Name:   MInvTopicDB.java
...
public class MInvTopicDB extends MInvTopicSchema
{
	...
	public MInvTopicDB()
	{
		...
		mStatement = null;
		con = null;
		db = new DBOper("MInvTopic");
		mflag = false;
	}

	public boolean deleteSQL()
	{
		MInvTopicSchema tSchema = getSchema();
	...
	public boolean delete()
	{
		...
		try
		{
			pstmt = con.prepareStatement("DELETE FROM MInvTopic WHERE  TopicID = ?");
	...
	public boolean insert()
	{
		...
		try
		{
			pstmt = con.prepareStatement("INSERT INTO MInvTopic VALUES( ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ?)");
			if (getTopicID() == null || getTopicID().equals("null"))
				pstmt.setNull(1, 12);
			else
				pstmt.setString(1, getTopicID());
	...
	public boolean getInfo()
	{
		...
		if (!mflag)
			con = DBConnPool.getConnection();
		int i;
		pstmt = con.prepareStatement("SELECT * FROM MInvTopic WHERE  TopicID = ?", 1003, 1007);
		if (getTopicID() == null || getTopicID().equals("null"))
			pstmt.setNull(1, 12);
		else
			pstmt.setString(1, getTopicID());
...
label0:
	...
	public MInvTopicSet query()
	{
		Statement stmt = null;
		ResultSet rs = null;
		MInvTopicSet aMInvTopicSet = new MInvTopicSet();
		if (!mflag)
			con = DBConnPool.getConnection();
		try
		{
			stmt = con.createStatement(1003, 1007);
			SQLString sqlObj = new SQLString("MInvTopic");
			MInvTopicSchema aSchema = getSchema();
			sqlObj.setSQL(5, aSchema);
			String sql = sqlObj.getSQL();
			rs = stmt.executeQuery(sql);
			int i = 0;
			MInvTopicSchema s1;
			for (; rs.next(); aMInvTopicSet.add(s1))
			{
				i++;
				s1 = new MInvTopicSchema();
				s1.setSchema(rs, i);
			}
	...
	public MInvTopicSet query(int nStart, int nCount)
	{
		...
		try
		{
			...
			while (rs.next()) 
			{
				...
				if (i >= nStart + nCount)
					break;
				MInvTopicSchema s1 = new MInvTopicSchema();
				s1.setSchema(rs, i);
				aMInvTopicSet.add(s1);
			}
			try
			{
				rs.close();
...
label0:
		{
			...
			try
			{
				...
				String sql = (new StringBuilder("update MInvTopic ")).append(sqlObj.getUpdPart()).append(" where ").append(strWherePart).toString();
	...
	{
		int tCount = 0;
		MInvTopicSet tMInvTopicSet = new MInvTopicSet();
		MInvTopicSchema tMInvTopicSchema = null;
		if (mResultSet == null)
		{
			CError tError = new CError();
			tError.moduleName = "MInvTopicDB";
		...
		{
			tCount = 1;
			tMInvTopicSchema = new MInvTopicSchema();
			tMInvTopicSchema.setSchema(mResultSet, 1);
			tMInvTopicSet.add(tMInvTopicSchema);
			while (tCount++ < 5000) 
				if (mResultSet.next())
				{
					tMInvTopicSchema = new MInvTopicSchema();
