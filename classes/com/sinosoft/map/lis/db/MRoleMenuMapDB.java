...
// Source File Name:   MRoleMenuMapDB.java
...
public class MRoleMenuMapDB extends MRoleMenuMapSchema
{
	...
	{
		MRoleMenuMapSchema tSchema = getSchema();
		...
		} else
		{
			...
			tError.moduleName = "MRoleMenuMapDB";
	...
	public boolean getInfo()
	{
		...
		if (!mflag)
			con = DBConnPool.getConnection();
		int i;
		pstmt = con.prepareStatement("SELECT * FROM MRoleMenuMap WHERE  ID = ?", 1003, 1007);
		if (getID() == null || getID().equals("null"))
			pstmt.setNull(1, 12);
		else
			pstmt.setString(1, getID());
...
label0:
	...
	public MRoleMenuMapSet query()
	{
		Statement stmt = null;
		ResultSet rs = null;
		MRoleMenuMapSet aMRoleMenuMapSet = new MRoleMenuMapSet();
		if (!mflag)
			con = DBConnPool.getConnection();
		try
		{
			stmt = con.createStatement(1003, 1007);
			SQLString sqlObj = new SQLString("MRoleMenuMap");
			MRoleMenuMapSchema aSchema = getSchema();
			sqlObj.setSQL(5, aSchema);
			String sql = sqlObj.getSQL();
			rs = stmt.executeQuery(sql);
			int i = 0;
			MRoleMenuMapSchema s1;
			for (; rs.next(); aMRoleMenuMapSet.add(s1))
			{
				i++;
				s1 = new MRoleMenuMapSchema();
				s1.setSchema(rs, i);
			}
	...
	public MRoleMenuMapSet query(int nStart, int nCount)
	{
		...
		try
		{
			...
			while (rs.next()) 
			{
				...
				if (i >= nStart + nCount)
					break;
				MRoleMenuMapSchema s1 = new MRoleMenuMapSchema();
				s1.setSchema(rs, i);
				aMRoleMenuMapSet.add(s1);
			}
			try
			{
				rs.close();
...
label0:
		{
			...
			try
			{
				...
				String sql = (new StringBuilder("update MRoleMenuMap ")).append(sqlObj.getUpdPart()).append(" where ").append(strWherePart).toString();
	...
	{
		int tCount = 0;
		MRoleMenuMapSet tMRoleMenuMapSet = new MRoleMenuMapSet();
		MRoleMenuMapSchema tMRoleMenuMapSchema = null;
		if (mResultSet == null)
		{
			CError tError = new CError();
			tError.moduleName = "MRoleMenuMapDB";
		...
		{
			tCount = 1;
			tMRoleMenuMapSchema = new MRoleMenuMapSchema();
			tMRoleMenuMapSchema.setSchema(mResultSet, 1);
			tMRoleMenuMapSet.add(tMRoleMenuMapSchema);
			while (tCount++ < 5000) 
				if (mResultSet.next())
				{
					tMRoleMenuMapSchema = new MRoleMenuMapSchema();
