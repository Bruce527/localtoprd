...
// Source File Name:   MTrainInfoDB.java
...
public class MTrainInfoDB extends MTrainInfoSchema
{
	...
	{
		MTrainInfoSchema tSchema = getSchema();
		...
		} else
		{
			...
			tError.moduleName = "MTrainInfoDB";
	...
	public boolean update()
	{
		...
		try
		{
			...
			if (getAgentName() == null || getAgentName().equals("null"))
				...
				pstmt.setString(3, getAgentName());
			if (getAgentGrade() == null || getAgentGrade().equals("null"))
				pstmt.setNull(4, 12);
			else
				pstmt.setString(4, getAgentGrade());
			if (getAgentGroup() == null || getAgentGroup().equals("null"))
				pstmt.setNull(5, 12);
			else
				pstmt.setString(5, getAgentGroup());
			if (getManageCom() == null || getManageCom().equals("null"))
				pstmt.setNull(6, 12);
			else
				pstmt.setString(6, getManageCom());
			if (getBranchType() == null || getBranchType().equals("null"))
				pstmt.setNull(7, 12);
			else
				pstmt.setString(7, getBranchType());
			if (getCourseID() == null || getCourseID().equals("null"))
				pstmt.setNull(8, 12);
			else
				pstmt.setString(8, getCourseID());
			if (getTrainAdd() == null || getTrainAdd().equals("null"))
				pstmt.setNull(11, 12);
			else
				pstmt.setString(11, getTrainAdd());
			if (getHours() == null || getHours().equals("null"))
				pstmt.setNull(12, 12);
			else
				pstmt.setString(12, getHours());
			if (getTeacher() == null || getTeacher().equals("null"))
				pstmt.setNull(13, 12);
			else
				pstmt.setString(13, getTeacher());
			if (getPassFlag() == null || getPassFlag().equals("null"))
				pstmt.setNull(14, 12);
			else
				pstmt.setString(14, getPassFlag());
			if (getScore() == null || getScore().equals("null"))
				pstmt.setNull(15, 12);
			else
				pstmt.setString(15, getScore());
	...
	public boolean getInfo()
	{
		...
		if (!mflag)
			con = DBConnPool.getConnection();
		int i;
		pstmt = con.prepareStatement("SELECT * FROM MTrainInfo WHERE  ID = ?", 1003, 1007);
		if (getID() == null || getID().equals("null"))
			pstmt.setNull(1, 12);
		else
			pstmt.setString(1, getID());
...
label0:
	...
	public MTrainInfoSet query()
	{
		Statement stmt = null;
		ResultSet rs = null;
		MTrainInfoSet aMTrainInfoSet = new MTrainInfoSet();
		if (!mflag)
			con = DBConnPool.getConnection();
		try
		{
			stmt = con.createStatement(1003, 1007);
			SQLString sqlObj = new SQLString("MTrainInfo");
			MTrainInfoSchema aSchema = getSchema();
			sqlObj.setSQL(5, aSchema);
			String sql = sqlObj.getSQL();
			rs = stmt.executeQuery(sql);
			int i = 0;
			MTrainInfoSchema s1;
			for (; rs.next(); aMTrainInfoSet.add(s1))
			{
				i++;
				s1 = new MTrainInfoSchema();
				s1.setSchema(rs, i);
			}
	...
	public MTrainInfoSet query(int nStart, int nCount)
	{
		...
		try
		{
			...
			while (rs.next()) 
			{
				...
				if (i >= nStart + nCount)
					break;
				MTrainInfoSchema s1 = new MTrainInfoSchema();
				s1.setSchema(rs, i);
				aMTrainInfoSet.add(s1);
			}
			try
			{
				rs.close();
...
label0:
		{
			...
			try
			{
				...
				String sql = (new StringBuilder("update MTrainInfo ")).append(sqlObj.getUpdPart()).append(" where ").append(strWherePart).toString();
	...
	{
		int tCount = 0;
		MTrainInfoSet tMTrainInfoSet = new MTrainInfoSet();
		MTrainInfoSchema tMTrainInfoSchema = null;
		if (mResultSet == null)
		{
			CError tError = new CError();
			tError.moduleName = "MTrainInfoDB";
		...
		{
			tCount = 1;
			tMTrainInfoSchema = new MTrainInfoSchema();
			tMTrainInfoSchema.setSchema(mResultSet, 1);
			tMTrainInfoSet.add(tMTrainInfoSchema);
			while (tCount++ < 5000) 
				if (mResultSet.next())
				{
					tMTrainInfoSchema = new MTrainInfoSchema();
