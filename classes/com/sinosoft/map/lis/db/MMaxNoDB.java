...
// Source File Name:   MMaxNoDB.java
...
public class MMaxNoDB extends MMaxNoSchema
{
	...
	{
		MMaxNoSchema tSchema = getSchema();
	...
	public boolean getInfo()
	{
		...
		if (!mflag)
			con = DBConnPool.getConnection();
		int i;
		pstmt = con.prepareStatement("SELECT * FROM MMaxNo WHERE  NoType = ? AND NoLimit = ?", 1003, 1007);
		if (getNoType() == null || getNoType().equals("null"))
			pstmt.setNull(1, 12);
		else
			pstmt.setString(1, getNoType());
...
label0:
	...
	public MMaxNoSet query()
	{
		Statement stmt = null;
		ResultSet rs = null;
		MMaxNoSet aMMaxNoSet = new MMaxNoSet();
		if (!mflag)
			con = DBConnPool.getConnection();
		try
		{
			stmt = con.createStatement(1003, 1007);
			SQLString sqlObj = new SQLString("MMaxNo");
			MMaxNoSchema aSchema = getSchema();
			sqlObj.setSQL(5, aSchema);
			String sql = sqlObj.getSQL();
			rs = stmt.executeQuery(sql);
			int i = 0;
			MMaxNoSchema s1;
			for (; rs.next(); aMMaxNoSet.add(s1))
			{
				i++;
				s1 = new MMaxNoSchema();
				s1.setSchema(rs, i);
	...
	public MMaxNoSet query(int nStart, int nCount)
	{
		...
		try
		{
			...
			while (rs.next()) 
			{
				...
				if (i >= nStart + nCount)
					break;
				MMaxNoSchema s1 = new MMaxNoSchema();
				s1.setSchema(rs, i);
				aMMaxNoSet.add(s1);
			}
			try
...
label0:
		{
			...
			try
			{
				...
				String sql = (new StringBuilder("update MMaxNo ")).append(sqlObj.getUpdPart()).append(" where ").append(strWherePart).toString();
	...
	{
		int tCount = 0;
		MMaxNoSet tMMaxNoSet = new MMaxNoSet();
		MMaxNoSchema tMMaxNoSchema = null;
		if (mResultSet == null)
		{
			CError tError = new CError();
			tError.moduleName = "MMaxNoDB";
		...
		{
			tCount = 1;
			tMMaxNoSchema = new MMaxNoSchema();
			tMMaxNoSchema.setSchema(mResultSet, 1);
			tMMaxNoSet.add(tMMaxNoSchema);
			while (tCount++ < 5000) 
				if (mResultSet.next())
				{
					tMMaxNoSchema = new MMaxNoSchema();
