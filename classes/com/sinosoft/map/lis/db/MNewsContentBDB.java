...
// Source File Name:   MNewsContentBDB.java
...
public class MNewsContentBDB extends MNewsContentBSchema
{
	...
	{
		MNewsContentBSchema tSchema = getSchema();
		...
		} else
		{
			...
			tError.moduleName = "MNewsContentBDB";
	...
	public boolean getInfo()
	{
		...
		if (!mflag)
			con = DBConnPool.getConnection();
		int i;
		pstmt = con.prepareStatement("SELECT * FROM MNewsContentB WHERE  EdorNo = ?", 1003, 1007);
		if (getEdorNo() == null || getEdorNo().equals("null"))
			pstmt.setNull(1, 12);
		else
			pstmt.setString(1, getEdorNo());
...
label0:
	...
	public MNewsContentBSet query()
	{
		Statement stmt = null;
		ResultSet rs = null;
		MNewsContentBSet aMNewsContentBSet = new MNewsContentBSet();
		if (!mflag)
			con = DBConnPool.getConnection();
		try
		{
			stmt = con.createStatement(1003, 1007);
			SQLString sqlObj = new SQLString("MNewsContentB");
			MNewsContentBSchema aSchema = getSchema();
			sqlObj.setSQL(5, aSchema);
			String sql = sqlObj.getSQL();
			rs = stmt.executeQuery(sql);
			int i = 0;
			MNewsContentBSchema s1;
			for (; rs.next(); aMNewsContentBSet.add(s1))
			{
				i++;
				s1 = new MNewsContentBSchema();
				s1.setSchema(rs, i);
			}
	...
	public MNewsContentBSet query(int nStart, int nCount)
	{
		...
		try
		{
			...
			while (rs.next()) 
			{
				...
				if (i >= nStart + nCount)
					break;
				MNewsContentBSchema s1 = new MNewsContentBSchema();
				s1.setSchema(rs, i);
				aMNewsContentBSet.add(s1);
			}
			try
			{
				rs.close();
...
label0:
		{
			...
			try
			{
				...
				String sql = (new StringBuilder("update MNewsContentB ")).append(sqlObj.getUpdPart()).append(" where ").append(strWherePart).toString();
	...
	{
		int tCount = 0;
		MNewsContentBSet tMNewsContentBSet = new MNewsContentBSet();
		MNewsContentBSchema tMNewsContentBSchema = null;
		if (mResultSet == null)
		{
			CError tError = new CError();
			tError.moduleName = "MNewsContentBDB";
		...
		{
			tCount = 1;
			tMNewsContentBSchema = new MNewsContentBSchema();
			tMNewsContentBSchema.setSchema(mResultSet, 1);
			tMNewsContentBSet.add(tMNewsContentBSchema);
			while (tCount++ < 5000) 
				if (mResultSet.next())
				{
					tMNewsContentBSchema = new MNewsContentBSchema();
