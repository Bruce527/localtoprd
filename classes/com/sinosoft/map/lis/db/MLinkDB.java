...
public class MLinkDB extends MLinkSchema
{
	...
	public boolean getInfo()
	{
		...
		if (!mflag)
			con = DBConnPool.getConnection();
		int i;
		pstmt = con.prepareStatement("SELECT * FROM MLink WHERE  ID = ?", 1003, 1007);
		if (getID() == null || getID().equals("null"))
			pstmt.setNull(1, 12);
		else
			pstmt.setString(1, getID());
...
label0:
	...
	public MLinkSet query()
	{
		Statement stmt = null;
		ResultSet rs = null;
		MLinkSet aMLinkSet = new MLinkSet();
		if (!mflag)
			con = DBConnPool.getConnection();
		try
		{
			stmt = con.createStatement(1003, 1007);
			SQLString sqlObj = new SQLString("MLink");
			MLinkSchema aSchema = getSchema();
			sqlObj.setSQL(5, aSchema);
			String sql = sqlObj.getSQL();
			rs = stmt.executeQuery(sql);
			int i = 0;
			MLinkSchema s1;
			for (; rs.next(); aMLinkSet.add(s1))
			{
				i++;
				s1 = new MLinkSchema();
				s1.setSchema(rs, i);
	...
	public MLinkSet query(int nStart, int nCount)
	{
		...
		try
		{
			...
			while (rs.next()) 
			{
				...
				if (i >= nStart + nCount)
					break;
				MLinkSchema s1 = new MLinkSchema();
				s1.setSchema(rs, i);
				aMLinkSet.add(s1);
			}
			try
...
label0:
		{
			...
			try
			{
				...
				String sql = (new StringBuilder("update MLink ")).append(sqlObj.getUpdPart()).append(" where ").append(strWherePart).toString();
	...
	{
		int tCount = 0;
		MLinkSet tMLinkSet = new MLinkSet();
		MLinkSchema tMLinkSchema = null;
		if (mResultSet == null)
		{
			CError tError = new CError();
			tError.moduleName = "MLinkDB";
		...
		{
			tCount = 1;
			tMLinkSchema = new MLinkSchema();
			tMLinkSchema.setSchema(mResultSet, 1);
			tMLinkSet.add(tMLinkSchema);
			while (tCount++ < 5000) 
				if (mResultSet.next())
				{
					tMLinkSchema = new MLinkSchema();
