...
// Source File Name:   MPrivateMessageDB.java
...
public class MPrivateMessageDB extends MPrivateMessageSchema
{
	...
	{
		MPrivateMessageSchema tSchema = getSchema();
		...
		} else
		{
			...
			tError.moduleName = "MPrivateMessageDB";
	...
	public boolean getInfo()
	{
		...
		if (!mflag)
			con = DBConnPool.getConnection();
		int i;
		pstmt = con.prepareStatement("SELECT * FROM MPrivateMessage WHERE  ID = ?", 1003, 1007);
		if (getID() == null || getID().equals("null"))
			pstmt.setNull(1, 12);
		else
			pstmt.setString(1, getID());
...
label0:
	...
	public MPrivateMessageSet query()
	{
		Statement stmt = null;
		ResultSet rs = null;
		MPrivateMessageSet aMPrivateMessageSet = new MPrivateMessageSet();
		if (!mflag)
			con = DBConnPool.getConnection();
		try
		{
			stmt = con.createStatement(1003, 1007);
			SQLString sqlObj = new SQLString("MPrivateMessage");
			MPrivateMessageSchema aSchema = getSchema();
			sqlObj.setSQL(5, aSchema);
			String sql = sqlObj.getSQL();
			rs = stmt.executeQuery(sql);
			int i = 0;
			MPrivateMessageSchema s1;
			for (; rs.next(); aMPrivateMessageSet.add(s1))
			{
				i++;
				s1 = new MPrivateMessageSchema();
				s1.setSchema(rs, i);
			}
	...
	public MPrivateMessageSet query(int nStart, int nCount)
	{
		...
		try
		{
			...
			while (rs.next()) 
			{
				...
				if (i >= nStart + nCount)
					break;
				MPrivateMessageSchema s1 = new MPrivateMessageSchema();
				s1.setSchema(rs, i);
				aMPrivateMessageSet.add(s1);
			}
			try
			{
				rs.close();
...
label0:
		{
			...
			try
			{
				...
				String sql = (new StringBuilder("update MPrivateMessage ")).append(sqlObj.getUpdPart()).append(" where ").append(strWherePart).toString();
	...
	public MPrivateMessageSet getData()
	{
		...
		MPrivateMessageSchema tMPrivateMessageSchema = null;
		if (mResultSet == null)
		{
			CError tError = new CError();
			tError.moduleName = "MPrivateMessageDB";
		...
		{
			tCount = 1;
			tMPrivateMessageSchema = new MPrivateMessageSchema();
			tMPrivateMessageSchema.setSchema(mResultSet, 1);
