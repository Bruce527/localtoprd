...
// Source File Name:   MHonorCheckDB.java
...
public class MHonorCheckDB extends MHonorCheckSchema
{
	...
	{
		MHonorCheckSchema tSchema = getSchema();
		...
		} else
		{
			...
			tError.moduleName = "MHonorCheckDB";
	...
	public boolean update()
	{
		...
		try
		{
			...
			if (getStatBatch() == null || getStatBatch().equals("null"))
				...
				pstmt.setString(6, getStatBatch());
	...
	public boolean getInfo()
	{
		...
		if (!mflag)
			con = DBConnPool.getConnection();
		int i;
		pstmt = con.prepareStatement("SELECT * FROM MHonorCheck WHERE  CheckID = ?", 1003, 1007);
		if (getCheckID() == null || getCheckID().equals("null"))
			pstmt.setNull(1, 12);
		else
			pstmt.setString(1, getCheckID());
...
label0:
	...
	public MHonorCheckSet query()
	{
		Statement stmt = null;
		ResultSet rs = null;
		MHonorCheckSet aMHonorCheckSet = new MHonorCheckSet();
		if (!mflag)
			con = DBConnPool.getConnection();
		try
		{
			stmt = con.createStatement(1003, 1007);
			SQLString sqlObj = new SQLString("MHonorCheck");
			MHonorCheckSchema aSchema = getSchema();
			sqlObj.setSQL(5, aSchema);
			String sql = sqlObj.getSQL();
			rs = stmt.executeQuery(sql);
			int i = 0;
			MHonorCheckSchema s1;
			for (; rs.next(); aMHonorCheckSet.add(s1))
			{
				i++;
				s1 = new MHonorCheckSchema();
				s1.setSchema(rs, i);
			}
	...
	public MHonorCheckSet query(int nStart, int nCount)
	{
		...
		try
		{
			...
			while (rs.next()) 
			{
				...
				if (i >= nStart + nCount)
					break;
				MHonorCheckSchema s1 = new MHonorCheckSchema();
				s1.setSchema(rs, i);
				aMHonorCheckSet.add(s1);
			}
			try
			{
				rs.close();
...
label0:
		{
			...
			try
			{
				...
				String sql = (new StringBuilder("update MHonorCheck ")).append(sqlObj.getUpdPart()).append(" where ").append(strWherePart).toString();
	...
	{
		int tCount = 0;
		MHonorCheckSet tMHonorCheckSet = new MHonorCheckSet();
		MHonorCheckSchema tMHonorCheckSchema = null;
		if (mResultSet == null)
		{
			CError tError = new CError();
			tError.moduleName = "MHonorCheckDB";
		...
		{
			tCount = 1;
			tMHonorCheckSchema = new MHonorCheckSchema();
			tMHonorCheckSchema.setSchema(mResultSet, 1);
			tMHonorCheckSet.add(tMHonorCheckSchema);
			while (tCount++ < 5000) 
				if (mResultSet.next())
				{
					tMHonorCheckSchema = new MHonorCheckSchema();
