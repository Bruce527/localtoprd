...
// Source File Name:   MHomePictureDB.java
...
public class MHomePictureDB extends MHomePictureSchema
{
	...
	public MHomePictureDB(Connection tConnection)
	{
		...
		mStatement = null;
		con = tConnection;
		db = new DBOper(con, "MHomePicture");
		mflag = true;
	}
	...
	public MHomePictureDB()
	{
		...
		mStatement = null;
		con = null;
		db = new DBOper("MHomePicture");
		mflag = false;
	}

	public boolean deleteSQL()
	{
		MHomePictureSchema tSchema = getSchema();
		...
		} else
		{
			...
			tError.moduleName = "MHomePictureDB";
	...
	public boolean delete()
	{
		...
		try
		{
			pstmt = con.prepareStatement("DELETE FROM MHomePicture WHERE  ID = ?");
	...
	public boolean update()
	{
		...
		try
		{
			pstmt = con.prepareStatement("UPDATE MHomePicture SET  ID = ? , URL = ? , RefURL = ? , Name = ? , Alt = ? , PicOrder = ? , DisplayFlag = ? , CheckState = ? , CheckOperator = ? , CheckReason = ? , CheckDate = ? , CheckTime = ? , VF01 = ? , NF01 = ? , IF01 = ? , DF02 = ? , Date01 = ? , ModifyOperator = ? , Operator = ? , MakeDate = ? , MakeTime = ? , ModifyDate = ? , ModifyTime = ? , Description = ? WHERE  ID = ?");
			if (getID() == null || getID().equals("null"))
				pstmt.setNull(1, 12);
			else
				pstmt.setString(1, getID());
			...
			if (getDescription() == null || getDescription().equals("null"))
				...
				pstmt.setString(24, getDescription());
			if (getID() == null || getID().equals("null"))
				pstmt.setNull(25, 12);
			else
	...
	public boolean insert()
	{
		...
		try
		{
			pstmt = con.prepareStatement("INSERT INTO MHomePicture VALUES( ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ?)");
			if (getID() == null || getID().equals("null"))
				pstmt.setNull(1, 12);
			else
				pstmt.setString(1, getID());
	...
	public boolean getInfo()
	{
		...
		if (!mflag)
			con = DBConnPool.getConnection();
		int i;
		pstmt = con.prepareStatement("SELECT * FROM MHomePicture WHERE  ID = ?", 1003, 1007);
		if (getID() == null || getID().equals("null"))
			pstmt.setNull(1, 12);
		else
			pstmt.setString(1, getID());
...
label0:
	...
	public MHomePictureSet query()
	{
		Statement stmt = null;
		ResultSet rs = null;
		MHomePictureSet aMHomePictureSet = new MHomePictureSet();
		if (!mflag)
			con = DBConnPool.getConnection();
		try
		{
			stmt = con.createStatement(1003, 1007);
			SQLString sqlObj = new SQLString("MHomePicture");
			MHomePictureSchema aSchema = getSchema();
			sqlObj.setSQL(5, aSchema);
			String sql = sqlObj.getSQL();
			rs = stmt.executeQuery(sql);
			int i = 0;
			MHomePictureSchema s1;
			for (; rs.next(); aMHomePictureSet.add(s1))
			{
				i++;
				s1 = new MHomePictureSchema();
				s1.setSchema(rs, i);
			}
	...
	public MHomePictureSet query(int nStart, int nCount)
	{
		...
		try
		{
			...
			while (rs.next()) 
			{
				...
				if (i >= nStart + nCount)
					break;
				MHomePictureSchema s1 = new MHomePictureSchema();
				s1.setSchema(rs, i);
				aMHomePictureSet.add(s1);
			}
			try
			{
				rs.close();
...
label0:
		{
			...
			try
			{
				...
				String sql = (new StringBuilder("update MHomePicture ")).append(sqlObj.getUpdPart()).append(" where ").append(strWherePart).toString();
	...
	{
		int tCount = 0;
		MHomePictureSet tMHomePictureSet = new MHomePictureSet();
		MHomePictureSchema tMHomePictureSchema = null;
		if (mResultSet == null)
		{
			CError tError = new CError();
			tError.moduleName = "MHomePictureDB";
		...
		{
			tCount = 1;
			tMHomePictureSchema = new MHomePictureSchema();
			tMHomePictureSchema.setSchema(mResultSet, 1);
			tMHomePictureSet.add(tMHomePictureSchema);
			while (tCount++ < 5000) 
				if (mResultSet.next())
				{
					tMHomePictureSchema = new MHomePictureSchema();
