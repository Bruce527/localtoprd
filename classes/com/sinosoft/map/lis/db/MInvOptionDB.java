...
// Source File Name:   MInvOptionDB.java
...
public class MInvOptionDB extends MInvOptionSchema
{
	...
	{
		MInvOptionSchema tSchema = getSchema();
		...
		} else
		{
			...
			tError.moduleName = "MInvOptionDB";
	...
	public boolean update()
	{
		...
		try
		{
			...
			if (getOption() == null || getOption().equals("null"))
				...
				pstmt.setString(3, getOption());
			pstmt.setInt(4, getOptionOrder());
			...
			if (getVF01() == null || getVF01().equals("null"))
				...
				pstmt.setString(6, getVF01());
			if (getNF01() == null || getNF01().equals("null"))
				pstmt.setNull(7, 12);
			else
				pstmt.setString(7, getNF01());
			pstmt.setInt(8, getIF01());
			pstmt.setDouble(9, getDF02());
			if (getDate01() == null || getDate01().equals("null"))
				pstmt.setNull(10, 91);
			else
				pstmt.setDate(10, Date.valueOf(getDate01()));
			if (getModifyOperator() == null || getModifyOperator().equals("null"))
				pstmt.setNull(11, 12);
			else
				pstmt.setString(11, getModifyOperator());
			...
			if (getOptionID() == null || getOptionID().equals("null"))
				...
				pstmt.setString(17, getOptionID());
			pstmt.executeUpdate();
			pstmt.close();
		}
		catch (Exception ex)
	...
	public boolean getInfo()
	{
		...
		if (!mflag)
			con = DBConnPool.getConnection();
		int i;
		pstmt = con.prepareStatement("SELECT * FROM MInvOption WHERE  OptionID = ?", 1003, 1007);
		if (getOptionID() == null || getOptionID().equals("null"))
			pstmt.setNull(1, 12);
		else
			pstmt.setString(1, getOptionID());
...
label0:
	...
	public MInvOptionSet query()
	{
		Statement stmt = null;
		ResultSet rs = null;
		MInvOptionSet aMInvOptionSet = new MInvOptionSet();
		if (!mflag)
			con = DBConnPool.getConnection();
		try
		{
			stmt = con.createStatement(1003, 1007);
			SQLString sqlObj = new SQLString("MInvOption");
			MInvOptionSchema aSchema = getSchema();
			sqlObj.setSQL(5, aSchema);
			String sql = sqlObj.getSQL();
			rs = stmt.executeQuery(sql);
			int i = 0;
			MInvOptionSchema s1;
			for (; rs.next(); aMInvOptionSet.add(s1))
			{
				i++;
				s1 = new MInvOptionSchema();
				s1.setSchema(rs, i);
			}
	...
	public MInvOptionSet query(int nStart, int nCount)
	{
		...
		try
		{
			...
			while (rs.next()) 
			{
				...
				if (i >= nStart + nCount)
					break;
				MInvOptionSchema s1 = new MInvOptionSchema();
				s1.setSchema(rs, i);
				aMInvOptionSet.add(s1);
			}
			try
			{
				rs.close();
...
label0:
		{
			...
			try
			{
				...
				String sql = (new StringBuilder("update MInvOption ")).append(sqlObj.getUpdPart()).append(" where ").append(strWherePart).toString();
	...
	{
		int tCount = 0;
		MInvOptionSet tMInvOptionSet = new MInvOptionSet();
		MInvOptionSchema tMInvOptionSchema = null;
		if (mResultSet == null)
		{
			CError tError = new CError();
			tError.moduleName = "MInvOptionDB";
		...
		{
			tCount = 1;
			tMInvOptionSchema = new MInvOptionSchema();
			tMInvOptionSchema.setSchema(mResultSet, 1);
			tMInvOptionSet.add(tMInvOptionSchema);
			while (tCount++ < 5000) 
				if (mResultSet.next())
				{
					tMInvOptionSchema = new MInvOptionSchema();
