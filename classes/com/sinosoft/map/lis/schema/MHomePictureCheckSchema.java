...
// Source File Name:   MHomePictureCheckSchema.java
	...
	implements Schema, Cloneable
{
	...
	{
		MHomePictureCheckSchema cloned = (MHomePictureCheckSchema)super.clone();
	...
	{
		iD = aMHomePictureCheckSchema.getID();
		uRL = aMHomePictureCheckSchema.getURL();
		refURL = aMHomePictureCheckSchema.getRefURL();
		name = aMHomePictureCheckSchema.getName();
		alt = aMHomePictureCheckSchema.getAlt();
		picOrder = aMHomePictureCheckSchema.getPicOrder();
		displayFlag = aMHomePictureCheckSchema.getDisplayFlag();
		checkState = aMHomePictureCheckSchema.getCheckState();
		checkOperator = aMHomePictureCheckSchema.getCheckOperator();
		checkReason = aMHomePictureCheckSchema.getCheckReason();
		checkDate = fDate.getDate(aMHomePictureCheckSchema.getCheckDate());
		checkTime = aMHomePictureCheckSchema.getCheckTime();
		vF01 = aMHomePictureCheckSchema.getVF01();
		nF01 = aMHomePictureCheckSchema.getNF01();
		iF01 = aMHomePictureCheckSchema.getIF01();
		dF02 = aMHomePictureCheckSchema.getDF02();
		date01 = fDate.getDate(aMHomePictureCheckSchema.getDate01());
		modifyOperator = aMHomePictureCheckSchema.getModifyOperator();
		operator = aMHomePictureCheckSchema.getOperator();
		makeDate = fDate.getDate(aMHomePictureCheckSchema.getMakeDate());
		makeTime = aMHomePictureCheckSchema.getMakeTime();
		modifyDate = fDate.getDate(aMHomePictureCheckSchema.getModifyDate());
		modifyTime = aMHomePictureCheckSchema.getModifyTime();
	}
	...
	public boolean setSchema(ResultSet rs, int i)
	{
		...
		{
			System.out.println("数据库中的MHomePictureCheck表字段个数和Schema中的字段个数不一致，或者执行db.executeQuery查询时没有使用select * from tables");
			CError tError = new CError();
			tError.moduleName = "MHomePictureCheckSchema";
	...
	{
		MHomePictureCheckSchema aMHomePictureCheckSchema = new MHomePictureCheckSchema();
		aMHomePictureCheckSchema.setSchema(this);
		return aMHomePictureCheckSchema;
	}

	public MHomePictureCheckDB getDB()
	{
		MHomePictureCheckDB aDBOper = new MHomePictureCheckDB();
	...
	public boolean equals(Object otherObject)
	{
		...
		if (getClass() != otherObject.getClass())
			return false;
		MHomePictureCheckSchema other = (MHomePictureCheckSchema)otherObject;
	...
	public int getFieldIndex(String strFieldName)
	{
		...
		if (strFieldName.equals("modifyDate"))
			return 21;
		return !strFieldName.equals("modifyTime") ? -1 : 22;
	}
