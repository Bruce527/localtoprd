...
// Source File Name:   MSysLinkSchema.java
	...
	implements Schema, Cloneable
{
	...
	{
		MSysLinkSchema cloned = (MSysLinkSchema)super.clone();
	...
	{
		iD = aMSysLinkSchema.getID();
		refURL = aMSysLinkSchema.getRefURL();
		target = aMSysLinkSchema.getTarget();
		name = aMSysLinkSchema.getName();
		linkOrder = aMSysLinkSchema.getLinkOrder();
		state = aMSysLinkSchema.getState();
		type = aMSysLinkSchema.getType();
		icon = aMSysLinkSchema.getIcon();
		manageCom = aMSysLinkSchema.getManageCom();
		branchType = aMSysLinkSchema.getBranchType();
		vF01 = aMSysLinkSchema.getVF01();
		nF01 = aMSysLinkSchema.getNF01();
		iF01 = aMSysLinkSchema.getIF01();
		dF02 = aMSysLinkSchema.getDF02();
		date01 = fDate.getDate(aMSysLinkSchema.getDate01());
		modifyOperator = aMSysLinkSchema.getModifyOperator();
		operator = aMSysLinkSchema.getOperator();
		makeDate = fDate.getDate(aMSysLinkSchema.getMakeDate());
		makeTime = aMSysLinkSchema.getMakeTime();
		modifyDate = fDate.getDate(aMSysLinkSchema.getModifyDate());
		modifyTime = aMSysLinkSchema.getModifyTime();
		description = aMSysLinkSchema.getDescription();
	}
	...
	public boolean setSchema(ResultSet rs, int i)
	{
		...
		{
			System.out.println("数据库中的MSysLink表字段个数和Schema中的字段个数不一致，或者执行db.executeQuery查询时没有使用select * from tables");
			CError tError = new CError();
			tError.moduleName = "MSysLinkSchema";
	...
	{
		MSysLinkSchema aMSysLinkSchema = new MSysLinkSchema();
		aMSysLinkSchema.setSchema(this);
		return aMSysLinkSchema;
	}

	public MSysLinkDB getDB()
	{
		MSysLinkDB aDBOper = new MSysLinkDB();
	...
	public boolean decode(String strMessage)
	{
		try
		{
			...
			icon = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 8, "|");
			manageCom = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 9, "|");
			branchType = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 10, "|");
			vF01 = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 11, "|");
			nF01 = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 12, "|");
			iF01 = (new Integer(ChgData.chgNumericStr(StrTool.getStr(strMessage, 13, "|")))).intValue();
			dF02 = (new Double(ChgData.chgNumericStr(StrTool.getStr(strMessage, 14, "|")))).doubleValue();
			date01 = fDate.getDate(StrTool.getStr(StrTool.GBKToUnicode(strMessage), 15, "|"));
			modifyOperator = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 16, "|");
			...
			description = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 22, "|");
		}
	...
	public boolean equals(Object otherObject)
	{
		...
		if (getClass() != otherObject.getClass())
			return false;
		MSysLinkSchema other = (MSysLinkSchema)otherObject;
	...
	public int getFieldIndex(String strFieldName)
	{
		...
		if (strFieldName.equals("modifyTime"))
			return 20;
		return !strFieldName.equals("description") ? -1 : 21;
	}
