...
// Source File Name:   MLibraryStructSchema.java
	...
	implements Schema, Cloneable
{
	...
	{
		MLibraryStructSchema cloned = (MLibraryStructSchema)super.clone();
	...
	{
		nodeID = aMLibraryStructSchema.getNodeID();
		name = aMLibraryStructSchema.getName();
		parentID = aMLibraryStructSchema.getParentID();
		discription = aMLibraryStructSchema.getDiscription();
		manageCom = aMLibraryStructSchema.getManageCom();
		branchType = aMLibraryStructSchema.getBranchType();
		position = aMLibraryStructSchema.getPosition();
		operation = aMLibraryStructSchema.getOperation();
		dirFlag = aMLibraryStructSchema.getDirFlag();
		fileFlag = aMLibraryStructSchema.getFileFlag();
		childDirNum = aMLibraryStructSchema.getChildDirNum();
		checkState = aMLibraryStructSchema.getCheckState();
		checkOperator = aMLibraryStructSchema.getCheckOperator();
		checkReason = aMLibraryStructSchema.getCheckReason();
		checkDate = fDate.getDate(aMLibraryStructSchema.getCheckDate());
		checkTime = aMLibraryStructSchema.getCheckTime();
		vF01 = aMLibraryStructSchema.getVF01();
		nF01 = aMLibraryStructSchema.getNF01();
		iF01 = aMLibraryStructSchema.getIF01();
		dF02 = aMLibraryStructSchema.getDF02();
		date01 = fDate.getDate(aMLibraryStructSchema.getDate01());
		modifyOperator = aMLibraryStructSchema.getModifyOperator();
		operator = aMLibraryStructSchema.getOperator();
		makeDate = fDate.getDate(aMLibraryStructSchema.getMakeDate());
		makeTime = aMLibraryStructSchema.getMakeTime();
		modifyDate = fDate.getDate(aMLibraryStructSchema.getModifyDate());
		modifyTime = aMLibraryStructSchema.getModifyTime();
	}
	...
	public boolean setSchema(ResultSet rs, int i)
	{
		...
		{
			System.out.println("数据库中的MLibraryStruct表字段个数和Schema中的字段个数不一致，或者执行db.executeQuery查询时没有使用select * from tables");
			CError tError = new CError();
			tError.moduleName = "MLibraryStructSchema";
	...
	{
		MLibraryStructSchema aMLibraryStructSchema = new MLibraryStructSchema();
		aMLibraryStructSchema.setSchema(this);
		return aMLibraryStructSchema;
	}

	public MLibraryStructDB getDB()
	{
		MLibraryStructDB aDBOper = new MLibraryStructDB();
	...
	public boolean equals(Object otherObject)
	{
		...
		if (getClass() != otherObject.getClass())
			return false;
		MLibraryStructSchema other = (MLibraryStructSchema)otherObject;
	...
	public int getFieldIndex(String strFieldName)
	{
		...
		if (strFieldName.equals("modifyDate"))
			return 25;
		return !strFieldName.equals("modifyTime") ? -1 : 26;
	}
